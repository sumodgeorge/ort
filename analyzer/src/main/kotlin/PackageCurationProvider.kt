 ...
 */
open class PackageCurationProvider {
    companion object {
        /**
         * A provider that does not provide any curations.
         */
        @JvmField
        val EMPTY = object : PackageCurationProvider() {
            override fun getCurationsFor(pkgIds: Collection<Identifier>) = emptyMap<Identifier, List<PackageCuration>>()
        }
    }
    ...
    open fun getCurationsFor(packages: Collection<Package>): Map<Identifier, List<PackageCuration>> =
        getCurationsFor(packages.map { it.id })

    internal open fun getCurationsFor(pkgIds: Collection<Identifier>): Map<Identifier, List<PackageCuration>> =
